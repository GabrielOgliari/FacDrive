...
class DashboardService implements IDashboardService {
    ...
    async createCarpoolDays({ id, days }: CarpoolDaysParams) {
        ...
        await axios<GetCarpoolDaysInput>({
            ...
            data: {
                idUser: id,
                monday: days.some(({ value, active }) => value === DaysOfTheWeek.Monday && active),
                tuesday: days.some(({ value, active }) => value === DaysOfTheWeek.Tuesday && active),
                wednesday: days.some(({ value, active }) => value === DaysOfTheWeek.Wednesday && active),
                thursday: days.some(({ value, active }) => value === DaysOfTheWeek.Thursday && active),
                friday: days.some(({ value, active }) => value === DaysOfTheWeek.Friday && active),
                saturday: days.some(({ value, active }) => value === DaysOfTheWeek.Saturday && active),
            },
        });
    }
