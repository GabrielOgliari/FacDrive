...
export const Confirmation = ({ onConfirm, onCancel }: ConfirmationProps) => {
    ...
    const { mutateAsync, isSuccess, isError } = useMutation({
        mutationFn: ({ confirmation }: { confirmation: boolean }) => dashboardService.confirmRide({ confirmation }),
    });
    ...
    return (
        <Container>
            <Text>Lembre-se de confirmar sua carona para casa. Confirmar agora? 🏠</Text>

            <Text type="light">Seu motorista: Gabriel Ogliari Roncato</Text>

            <S.Flex>
                <S.ConfirmButton onPress={onConfirm}>
                    <S.TextButton onPress={() => handleStatus(true)}>Confirmar</S.TextButton>
                </S.ConfirmButton>
                <S.CancelButton onPress={onCancel}>
                    <S.TextButton onPress={() => handleStatus(false)}>Cancelar</S.TextButton>
                </S.CancelButton>
            </S.Flex>
            ...
            {isError && (
                <S.RedContainer>
                    <S.Text>
                        Erro ao {status ? 'confirmar' : 'cancelar'} carona. Por favor, aguarde alguns minutos e tente
                        novamente.
                    </S.Text>
                </S.RedContainer>
            )}
