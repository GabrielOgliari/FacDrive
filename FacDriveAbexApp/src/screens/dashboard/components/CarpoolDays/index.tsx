...
export const CarpoolDays = () => {
    ...
    const createCarpoolDaysMutation = useMutation({
        mutationFn: (days: Days) => dashboardService.createCarpoolDays({ id: user.id, days }),
        onError: () =>
            dispatchToast({
    ...
    const updateCarpoolDaysMutation = useMutation({
        mutationFn: (days: Days) => dashboardService.updateCarpoolDays({ id: user.id, days }),
        onError: () =>
            dispatchToast({
    ...
    const setCarpoolDays = (value: DaysOfTheWeek) => {
        ...
        const updatedData = data.map(item => (item.value === value ? { ...item, active: !item.active } : item));
            ...
            <Container>
                ...
                <S.Days>
                    {data?.map(({ day, active, value }) => {
                        return (
                            <S.DayItem key={day} $isActive={active} onPress={() => setCarpoolDays(value)}>
