...
export const AccessDataScreen = () => {
    ...
    const { object, register, applyValidations, watch } = useForm<AccessDataForm>({
        validations: {
            institutionalEmail: value => {
                if (isEmpty(value)) return 'Por favor, insira o seu e-mail institucional.';
                if (!isValidInstitutionalEmail(value)) return 'Por favor, insira um e-mail institucional válido.';
            },
            password: value => {
                if (isEmpty(value)) return 'Por favor, insira a sua senha.';
            },
            passwordConfirmation: value => {
                if (isEmpty(value)) return 'Por favor, insira a sua senha de confirmação.';
                if (watch('password') !== watch('passwordConfirmation')) return 'As senhas devem ser iguais.';
            },
        },
    });

    const verifyEmailAlreadyRegisteredMutation = useMutation({
        mutationKey: ['verify-email-already-registered', watch('institutionalEmail')],
        mutationFn: () => signUpService.verifyEmailAlreadyRegistered(watch('institutionalEmail') as string),
    });
    ...
    const handlePressContinueButton = async () => {
        ...
        try {
            const { emailAlreadyRegistered } = await verifyEmailAlreadyRegisteredMutation.mutateAsync();
    ...
    return (
        <MainTemplate title="Vamos Começar o Seu Cadastro">
            ...
            <View style={{ gap: width * 0.08 }}>
                <Fields.Input placeholder="Email Institucional" {...register('institutionalEmail')} />

                <Fields.Input placeholder="Senha" mode="password" {...register('password')} />
